...
//            System.out.println(xy[1]);  
    ...
    public double[] getRegression(double[] xData, double[] yData) {

        for (int i = 0; i < xData.length; ++i) {
            addDataPoint(new DataPoint((float) xData[i], (float) yData[i]));
        }

//            System.out.println("\n回归线公式:  y = " + getA1() + "x + "+ getA0());
//            System.out.println("误差：     R^2 = " + getR());
        //calculate regresion y
        double[] regressionY = new double[xData.length];

        for (int i = 0; i < xData.length; ++i) {
            regressionY[i] = getA1() * xData[i] + getA0();
        }
